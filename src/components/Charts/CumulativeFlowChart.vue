...
export default {
  ...
  props: {
    stories: {
      type: Array,
      required: true,
      // элементы: { id, title, status, createdAt, endDate }
    }
  },
  computed: {
    series() {
      ...
      this.stories.forEach(story => {
        ...
        const ended = story.endDate ? story.endDate.split('T')[0] : null;

        if (created) dateCounts[created] = dateCounts[created] || { todo: 0, progress: 0, done: 0 };
        if (ended) dateCounts[ended] = dateCounts[ended] || { todo: 0, progress: 0, done: 0 };
      });

      const sortedDates = Object.keys(dateCounts).sort((a, b) => new Date(a) - new Date(b));

      // На каждую дату посчитаем активные истории
    ...
    chartOptions() {
      return {
        ...
        xaxis: {
          categories: this.series.length ? this.series[0].data.map((_, index) => this.sortedDates[index]) : [],
          labels: { rotate: -45 }
        },
        legend: {
          position: 'top'
